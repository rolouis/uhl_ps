...
from utils.approximation import packed_compress_func, nearest_quality_worker, compress_img


def get_target_qualities(file_path, target_jpeg_qualities=[80, 85, 90, 95, 100], experiment_encoders=[Encoder.JP2, Encoder.JXR, Encoder.JXL, Encoder.JPG, Encoder.HEIF, Encoder.WEBP, Encoder.AVIF, Encoder.BPG], max_workers=8):
    # Get target JPEG sizes in parallel

    target_jpeg_sizes = get_jpeg_targetsizes(file_path, max_workers, target_jpeg_qualities)
...
def get_folder_input_stats(dir_path, target_sizes= [80, 85, 90, 95, 100]):
    """
    Get the target sizes for all images in a folder
    :param dir_path:
    :param target_sizes:
    :return:
    """
...
def wsefgnerginnipgwe():
    ...
    for key, val in input_stats.items():
        get_target_qualities(key, experiment_encoders=[Encoder.JP2, Encoder.JXR, Encoder.JXL, Encoder.JPG, Encoder.HEIF, Encoder.WEBP, Encoder.AVIF, Encoder.BPG])
